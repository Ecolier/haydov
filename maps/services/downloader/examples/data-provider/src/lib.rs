...
struct DataProviderComponent;

impl Guest for DataProviderComponent {
    fn parse_urls(schema: Vec<u8>) -> Vec<String> {
        let schema: GeofabrikSchema = serde_json::from_slice(&schema)
            .expect("Failed to parse configuration as JSON or YAML");
